...
![窗口显示整体流程](/img/android/window_rendering/01_overview.svg)
...
![窗口层级树结构](/img/android/window_rendering/04_window_hierarchy.svg)
...
![addWindow 流程](/img/android/window_rendering/05_add_window_flow.svg)
...
![relayoutWindow 流程](/img/android/window_rendering/06_relayout_window_flow.svg)
...
![Surface 类型对比](/img/android/window_rendering/03_surface_types.svg)
...
关键步骤：
1. `win.makeSurface()` 方法会构建一个 Surface Builder
2. `.setBLASTLayer()` 无条件将其设置为 **Buffer 类型**
...
![layout 计算流程](/img/android/window_rendering/09_layout_compute_frames.svg)
>
![performSurfacePlacement 流程](/img/android/window_rendering/10_perform_surface_placement.svg)
...
![finishDrawingWindow 流程](/img/android/window_rendering/07_finish_drawing_flow.svg)
...
![Surface 显示步骤](/img/android/window_rendering/11_surface_display_steps.svg)
...
![Surface 状态流转](/img/android/window_rendering/02_surface_state.svg)
...
> 注：`SurfaceControl.closeTransaction()` 在当前版本中已是空方法（no-op），全局事务（`sGlobalTransaction`、`GlobalTransactionWrapper`）已不存在。事务提交通过各 `DisplayContent` 的 `SyncTransaction.apply()` 完成。

**统一提交的好处**：把一次 layout 中对所有窗口的 Surface 操作放在一个 Transaction 里做提交，避免逐个提交浪费资源和导致显示不同步。

### 3.8 finishDrawingWindow 小结

![完整 finishDrawingWindow 流程](/img/android/window_rendering/07_finish_drawing_flow.svg)
...
2. `SyncTransaction.apply()` —— 由 `RootWindowContainer` 统一提交
...
![应用端调用链](/img/android/window_rendering/08_app_side_call_chain.svg)
