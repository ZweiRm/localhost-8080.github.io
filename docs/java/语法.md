...
+ 二进制：0 ~ 1，满二进一，从Java SE 7.0 开始，允许在程序中标识一个二进制数，以`0b`或`0B`开头。如：`0b0011` = `0B0011`
...
在 Java SE 7.0, 新增了对`String`类进行判定。  
此外，`switch`还可以判定`Enum`枚举类型和一些特定的包装类，如`Character`, `Byte`, `Short`和`Integer`.  
